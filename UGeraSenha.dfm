object Form1: TForm1
  Left = 273
  Top = 220
  Width = 207
  Height = 147
  Caption = 'FIESPDB'
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  Icon.Data = {
    }
  OldCreateOrder = False
  PixelsPerInch = 96
  TextHeight = 13
  object Label1: TLabel
    Left = 12
    Top = 48
    Width = 56
    Height = 13
    Caption = 'Autorização'
  end
  object Label2: TLabel
    Left = 12
    Top = 8
    Width = 146
    Height = 13
    Caption = 'Número informado pelo usuário'
  end
  object Button1: TButton
    Left = 112
    Top = 88
    Width = 75
    Height = 25
    Caption = '&Ok'
    TabOrder = 0
    OnClick = Button1Click
  end
  object Edit1: TEdit
    Left = 12
    Top = 20
    Width = 173
    Height = 21
    CharCase = ecUpperCase
    TabOrder = 1
  end
  object Edit2: TEdit
    Left = 12
    Top = 60
    Width = 173
    Height = 21
    TabOrder = 2
  end
  object MSystemInfo1: TMSystemInfo
    About = 
      'MiTeC System Information Component 5.6 - Copyright © 1997,2001 M' +
      'ichal Mutl'
    CPU.FDIVBug = False
    CPU.CodeName = 'Mendocino (0.25 µm)'
    CPU.Transistors = 19000000
    Devices.DeviceCount = 75
    Processes.ProcessCount = 25
    Printers.DefaultIndex = 0
    Left = 100
    Top = 65520
  end
end
